...
export const setNestedValue = (obj, pathParts, value) => {
  ...
  for (let i = 0; i < pathParts.length - 1; i++) {
    ...
    const isNextPartArrayIndex =
      typeof nextPart === "number" ||
      (typeof nextPart === "string" && /^\d+$/.test(nextPart));
    ...
    // If it exists but is the wrong type (e.g., should be array but is object)
      ...
      console.warn(
        `Path conflict in setNestedValue: expected array at '${part}', found object. Overwriting.`
      );
      current[part] = [];
    } else if (!isNextPartArrayIndex && Array.isArray(current[part])) {
      console.warn(
        `Path conflict in setNestedValue: expected object at '${part}', found array. Overwriting.`
      );
      current[part] = {};
    }
...
export const updateArrayByPath = (currentState, path, arrayModifierFn) => {
  ...
  if (!Array.isArray(originalArray)) {
    ...
    console.warn(
      `Initializing non-array or missing path '${path}' as empty array.`
    );
